...
N_DEMO_SHOT=1
for N_TRAIN_SHOT in 16 32 64 128 256; do
    python3 saicl-multi-gpu.py \
        --llm_dir ${LLM_DIR} \
        --dataset ${DATASET} \
        --data_dir ${DATA_DIR} \
...
for DATASET in  mr rte trec; do #sst2 subj mpqa agnews cb cr dbpedia 


N_DEMO_SHOT=1
for N_TRAIN_SHOT in 4 8 16 32 64 128 256; do
    python3 saicl-multi-gpu.py \
        --llm_dir ${LLM_DIR} \
        --dataset ${DATASET} \
        --data_dir ${DATA_DIR} \
...
# nohup bash scripts/llama-30B.sh 2>&1 >>scripts/llama-30B.log &