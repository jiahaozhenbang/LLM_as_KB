...
def main():
    ...
    def classification_by_feature(feature_ids, feature_name=args.feature_choose):
        ...
        for name, clf in zip(names, classifiers):
            candidate_methods_for_all_feature = ["Nearest Neighbors", "Logistic Regression"]
            if (feature_name == 'all' or (not args.w_all_classifier)) and  name not in candidate_methods_for_all_feature:
                continue
            logger.info(f"Classification by {name}")
            start_time =  time.time()
            clf = make_pipeline(StandardScaler(), clf) if not args.no_StandardScaler else make_pipeline(clf)
            ...
            with open(save_results_file, 'a+', newline='') as csvfile:
                ...
                if not csv_exists:
                    csvwriter.writerow(['dataset', 'llm', 'n_train_shot', 'method', 'seed', 'acc', 'running_time'])
