...
class TrainsetStore(nn.Module):

    def __init__(self, K=1024, dim=50257, knn=1, n_class=2):
        super(TrainsetStore, self).__init__()
    ...
    def get_data(self, chosen_indexes):

        x = self.queue_anchor[:, chosen_indexes].cpu().numpy().tolist()
        y = self.queue_label.cpu().numpy().tolist()
        return x, y
