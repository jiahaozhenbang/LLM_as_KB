...
for N_TRAIN_SHOT in 4 8 16 32 64 128 256 512 1024; do
    ...
    python3 ablation_other_classifiers.py \
        --llm_dir ${LLM_DIR} \
        --dataset ${DATASET} \
        --data_dir ${DATA_DIR} \
        --n_train_shot ${N_TRAIN_SHOT} \
        --n_demo_shot ${N_DEMO_SHOT} \
        --seed ${SEED} \
        --output_dir ./output/ablation_other_classifiers/${LLM} \
        --feature_choose all
    done
done
...
# nohup bash ablation_other_classifiers.sh 2>&1 >output/ablation_other_classifiers.log &